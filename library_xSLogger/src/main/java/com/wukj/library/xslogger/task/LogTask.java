 ...
 */
package com.wukj.library.xslogger.task;

import android.content.Context;
import android.text.TextUtils;
import android.util.Log;

import com.wukj.library.xslogger.utils.FileUtils;
import com.wukj.library.xslogger.utils.LogUtils;
import com.wukj.library.xslogger.utils.Utils;
...
public class LogTask implements Runnable {
	...
	public void run() {
		...
		if ((mLogType & LogUtils.LOG_TYPE_2_TRACE) > 0) {
			traceLog2File("ms:" + System.currentTimeMillis() + " [" + mTag + "]"
					+ mMsg);
		}
	...
	private void traceLog2File(String msg) {
		Log.e("traceLog2File","Utils.sdAvailible():"+Utils.sdAvailible());
		if (Utils.sdAvailible()) {
			try {
				File file = new File(LogUtils.getTraceFileName(mContext));
				if (!file.exists() || file.isDirectory()) {
					FileUtils.delete(file);
					file.createNewFile();
				}

				Log.e("traceLog2File",file.getAbsolutePath());
				...
				OutputStreamWriter writer = new OutputStreamWriter(trace,
						"utf-8");
				Log.e("traceLog2File","日志内容："+Utils.encrypt(buildLog(msg, "")));
				writer.write(Utils.encrypt(buildLog(msg, "")));
				writer.flush();
